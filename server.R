...
# Cálculos iniciales ----
  ...
  data <- read_rds("data/data.rds") %>%
    dplyr::select(fecha, delito, x, y, colonia, municipio, clave_mun, hora, violencia, comision, victimas, tipo, zona_geografica) %>%
...
# Diseño de filtros ----  
  ...
  output$crimes <- renderUI({
    ...
    if(is.null(input$good)){
      selectInput("crime", multiple = T,
                  label = "Delitos",
                  choices = unique(data$delito)
      )
    } else {
      selectInput("crime", multiple = T,
                  label = "Delitos",
                  choices = unique(data$delito[data$bien_afectado %in% input$good])
  ...
  output$y_axis_ui <- renderUI({
    inputs <- if(length(input$mun) < 2 & !is.null(input$mun)){
      c("Fecha", "Mes", "Año",
        "Municipio", "Colonia",
    ...
    selectInput("y_axis",
                ...
                selected = ifelse(input$x_axis != "Total", "Total", "")
    )
  })
  ...
  output$ui_x_axis2 <- renderUI({
    selectInput("x_axis2",
                label = "Agrupación",
                choices = if(length(input$mun) < 2 & !is.null(input$mun)){
                  c("Fecha", "Mes", "Año",
  ...
  output$ui_x_axis <- renderUI({
                  ...
                  c("Fecha", "Mes", "Año",
                    ...
                    "Total")} else {
                      c("Fecha", "Mes", "Año",
                        ...
                        "Total")}
                ,
                selected = ifelse(input$graph_type!="Dispersión", input$x_axis2, "Mes"))
  })
    ...
    output$groupslist <- renderUI({
      ...
      if(length(input$mun)<2 & !is.null(input$mun)){
    ...
    output$ymeans <- renderUI({
      ...
      vl <- "input.graph_type == 'Dispersión' & input.cb_groups == false &
...
# Elementos de la barra superior: ----
  ...
  output$last <- renderText({
    str_to_sentence(paste(month(last_month, label = T, abbr = F), last_year))
  })
  
  output$period <- renderText({
    paste(str_to_sentence(month(last_month, label = T, abbr = T)), last_year-1,
                          "-",
          str_to_sentence(month(last_month-1, label = T, abbr = T)), last_year)
  })
...
# Botones: ----
  ...
  output$dwnld <- downloadHandler(
    ...
    content = function(file){
      ...
      filtered() %>%
        select(-c(violencia, comision, victimas, tipo)) %>%
  ...
  output$graph <- renderPlotly({
    ...
    base <- filtered() %>%
      ...
      setNames(c("date", "crime", "neighborhood", "mun",
                 ...
                 "type", "zona_geografica","good",  "month_of_year", "year", "year2",
                 "weekday", "month"))  %>%
        ...
        graph <- ggplot(cmplt,
          ...
          if(xaxis=="month"){
            base_means <- filtered_means() %>%
              ...
              setNames(c("date", "crime", "neighborhood", "mun",
                         "time", "violence", "com", "victims",
                         "type", "zona_geografica", "good", "month_of_year", "year", "month"))  %>%
              group_by(month, year) %>% 
              #summarise(total= ifelse(input$class == "Carpetas", n(), sum(victims))) %>%
              summarise(total = n()) %>%
              mutate(c_meses=n()) %>%
              ...
              summarise(total=sum(total),
                        c_meses=n()) %>%
              mutate(promedio=total/c_meses)
              
            graph <- ggplot(cmplt,
                            aes_string(x=xaxis, y=yaxis)
                            ) +
                              geom_hline(data=base_means %>% filter(year==2017),
                                         aes(yintercept = promedio, text=paste(round(promedio), "\n",year),
                                             colour="2017"#, colour = "2017"
                                             ),
                                         linetype="longdash"#,colour="#440154FF"
                                        ) +
                              geom_hline(data=base_means %>% filter(year==2018),
                                        aes(yintercept = promedio, text=paste(round(promedio), "\n",year),
                                            colour="2018"#, colour = "2018"
                                            ),
                                         linetype="longdash"#,colour="#39568CFF"
                                        ) +
                              geom_hline(data=base_means %>% filter(year==2019),
                                        aes(yintercept = promedio, text=paste(round(promedio), "\n",year),#, colour = "2019"
                                            colour="2019"),#, colour="#1F968BFF",
                                        linetype="longdash"
                                        ) +
                              geom_hline(data=base_means %>% filter(year==2020),
                                        aes(yintercept = promedio, text=paste(round(promedio), "\n",year),#, colour = "2020"
                                            colour="2020"), 
                                         linetype="longdash"#, colour="#73D055FF"
                                        ) + scale_color_manual(values = c("#1F968BFF", "#39568CFF", "#440154FF", "#1F968BFF")) +
              labs(colour="")
            ...
            graph <- ggplot(cmplt,
              ...
              geom_hline(data=base_means %>% filter(year==2017),
                        ...
                        colour="#440154FF", linetype="longdash") +
              geom_hline(data=base_means %>% filter(year==2018),
                        aes(yintercept = promedio, text=paste(round(promedio), "\n",year)
                            ), 
                        colour="#39568CFF", linetype="longdash") +
              geom_hline(data=base_means %>% filter(year==2019),
                        aes(yintercept = promedio, text=paste(round(promedio), "\n",year)
                            ), 
                        colour="#1F968BFF", linetype="longdash") +
              geom_hline(data=base_means %>% filter(year==2020),
                        aes(yintercept = promedio, text=paste(round(promedio), "\n",year)
                            ), 
                        colour="#73D055FF", linetype="longdash")
          }
            ...
            geom_point(size=1.5, alpha=.35, 
                       aes(text = paste(.data[[group]],
                                        ...
                                        paste0(year(.data[[xaxis]]), "-",
                                               month(.data[[xaxis]], abbr = T, label = T))
                       )
                       )
            )
        } else {
...
# Mapa: ----
  ...
  observe({
    ...
    map_filtered <- filtered() %>%
      drop_na(x) %>%
      mutate(dia_semana = factor(weekdays(fecha, F), 
                              levels = c("lunes", "martes", "miércoles",
                                         "jueves", "viernes", "sábado", "domingo")),
             label=paste(sep = "<br/>", 
